...
이때 **모델 성능은 양방향으로 측정을 망칠 수 있다**:

- **상한 — 모델이 인터페이스 결함을 가린다(거짓 음성)**: 강한 모델은 모호한 tool
  설명·이름이라도 추론력으로 "알아서" 올바르게 써버린다. 결함이 있는데 통과하므로
  인터페이스 문제가 드러나지 않는다. 프론티어 모델만 쓰면 여기 걸린다.
- **하한 — 모델이 정상인 인터페이스를 결함으로 만든다(거짓 양성)**: 너무 약한 모델은
  충분히 명확한 지침도 행동으로 옮기지 못한다. 인터페이스는 멀쩡한데 실패하므로
  "설명이 불명확하다"는 잘못된 결론으로 이어진다. 약한 모델만 쓰면 여기 걸린다.

**즉 단일 모델로는 어느 쪽도 판정할 수 없다.** 두 모델의 결과가 갈리는 지점 자체가
진단 신호다:

| `gemma-4-12b-it`(하한 탐침) | `qwen/qwen3.8-27b`(상한 참조) | 판정 |
|---|---|---|
| 실패 | 실패 | 인터페이스 결함일 가능성이 높다 → 고칠 대상 |
| 실패 | 통과 | 모델 한계 → **인터페이스를 건드리지 말 것** |
| 통과 | 통과 | 인터페이스 정상. 단 상한에 가려졌을 가능성은 남는다 |

기본 모델은 `gemma-4-12b-it`로 둔다(하한 탐침이 이 eval의 원래 목적). 다만 **gemma
단독 실패는 크로스 체크 전까지 인터페이스 결함의 근거로 쓰지 않는다** — 이건 운영
규칙이다. 실제로 이 규칙이 없던 시기에 gemma 단독 실패를 인터페이스 신호로 읽고
멀쩡한 인터페이스를 고치려 든 사례가 있었다(위키 연구 노트 §11 정정 참고).

**실측 근거**: 동일 코드·동일 tool description·동일 중립 프롬프트로 `keyword_search`
실행 시 — `gemma-4-12b-it` 0/5(검증용 `get_session` 호출 0회), `qwen/qwen3.8-27b`
3/3(매번 후보 2건을 모두 읽고 decoy를 정확히 배제). 같은 인터페이스에 대해 두 모델이
정반대 결과를 냈고, 이 차이가 "인터페이스 결함"과 "모델 한계"를 갈라줬다.
...
3. `gemma-4-12b-it` — `capabilities`에 `tool_use`가 명시돼 있고, `max_context_length`와
   동일한 131072로 로드됨(여유 충분). **하한 탐침**으로 현재 기본값.
4. `qwen/qwen3.8-27b`(27B, 80128 컨텍스트로 로드) — **상한 참조로 추가. gemma를
   대체하는 게 아니라 브래킷의 반대쪽이다.** 위 표의 판정을 내리려면 두 축이 다
   있어야 한다.

**같은 계열 더 소형인 `gemma-4-e4b-it`을 쓰지 않는 이유**: 로컬에 받아져 있고
`tool_use`도 명시돼 있지만 대상으로 삼지 않는다. 정직하게 적자면 처음부터 따져보고
배제한 게 아니라 그냥 고려하지 않았다 — 다만 위 실측으로 근거가 생겼다.
`gemma-4-12b-it`이 이미 tool description 수준의 지침을 행동으로 옮기지 못하는 상태
(0/5, `get_session` 0회)라 **하한선에 걸쳐 있거나 이미 그 아래**다. 같은 계열 더
작은 모델은 하한을 더 깊이 위반해서 거짓 양성만 늘린다 — 인터페이스 품질이 아니라
모델의 무능을 측정하게 된다. 하한 탐침은 "약할수록 좋다"가 아니라 "정상인 인터페이스는
따라올 수 있을 만큼은 되면서, 결함은 드러낼 만큼 약한" 지점이어야 한다.
...
**모델 일반화 불가.** 이 하네스가 만들어낸 결과는 실제로 돌려본 로컬 모델
(`gemma-4-12b-it`, `qwen/qwen3.8-27b`, 그리고 이전에 시도했던 `gpt-oss-20b`/
`meta/muse-glimmer`)에 대한 진술일 뿐, GPT-4o·Claude·Gemini나 다른 양자화 버전이
...
높지만, 이건 개연성 있는 추측이지 증명이 아니다.

다만 **크로스 모델 비교 자체는 이제 수행했다**(위 "상한/하한 브래킷" 절) — 이전
문서에 있던 "크로스 모델 비교 0건, 후속 작업으로 미룸"이라는 서술은 낡았다.
`gemma-4-12b-it`과 `qwen/qwen3.8-27b` 두 축으로 같은 태스크를 돌려 "인터페이스
결함"과 "모델 한계"를 실제로 갈라낸 사례가 있다. 남은 한계는 **두 축뿐이고 전체
스위트에 대한 정식 매트릭스 비교는 아직 없다**는 것 — 지금까지의 크로스 체크는
문제가 된 개별 태스크에 대한 단발성 진단이었다.
...
- 전체 스위트를 두 모델에 자동으로 돌려 나란히 비교하는 **매트릭스 실행은 없다** —
  `--model`로 수동 지정해 개별 태스크를 크로스 체크하는 방식이다(위 "상한/하한
  브래킷" 참고). 브래킷 판정이 필요한 태스크에만 선택적으로 적용한다.
