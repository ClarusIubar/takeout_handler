...
SYSTEM_PROMPT = (
    ...
    "search_sessions로 후보가 2개 이상 나오면, 최종 답변을 하기 전에 각 후보를 전부 "
    "get_session으로 불러 전체 내용을 확인해라. 예외 없이 항상 그렇게 해라 — "
    "matched_snippet만 보고 답하지 마라. 각 후보를 확인한 뒤에는 최종 답변을 쓰기 전에 "
    "후보마다 '이 후보가 실제로 질문과 관련 있는가: 예/아니오'를 스스로 명시적으로 "
    "판단해라. '아니오'로 판단한 후보는 최종 답변에 정답으로 포함하지 마라. "
    "사용자가 '어디 있는지' 물으면 해당 대화의 session_id를 "
    "답변에 명시해라. 찾는 내용이 없으면 없다고 솔직히 답하고, 다른 대화를 갖다 붙이지 "
    "마라."
)
